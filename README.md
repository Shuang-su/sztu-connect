...
项目目前以 Codex Plugin 和本地工具提供能力：你提供事件与来源，Agent 协助整理记录，关联人物、组织、地点、制度与主题；已合法导出的聊天记录也可由 Agent 按仓库格式整理，再用本地工具校验并重建为 HTML。你核对内容，本地工具负责校验并生成时间线与双向链接。记录保留出处、时间和上下文，让事实、回忆与不同说法能够被区分、追溯和补充。
...
- **多种读法**：同一批事件可组织为编年体、纪传体、典制体、纪事本末体或其他专题集合，不复制事实。
- **本地校验与导出**：通过 Codex Plugin 工作流和本地 CLI 校验记录、重建索引，并生成供应商无关的知识 JSONL；embedding 默认留在本地，不进入 Git。
- **分级隐私检查**：秘密和高风险直接标识会阻断；普通联系方式等提示核查，不要求默认脱敏。扫描不能替代对图片、视频或材料公开范围的人工审查。
...
2. **什么时候发生**：提供已知日期或大致范围，并说明依据；不知道的部分保持未知，不为通过检查补造日期。
3. **有什么材料**：提供来源链接、文件说明或引用位置；相关文字、高清图片、视频、聊天导出或 Computer History 线索可以互相补充，但需要逐项核查来源与公开范围。

原件由你自行妥善保管；不要直接把私人聊天、完整电脑活动流或未核查的原图上传到公开仓库。公开记录可以只保留允许公开的来源元数据，不代表本项目代为保存原件。
...
**聊天记录**：先自行合法取得导出文件，再请 Agent 按 [聊天导出说明](docs/CHAT_IMPORT.md) 映射到统一 JSONL，保留消息 ID、原时间、回复关系与上下文。仓库内的 `validate-chat` 可校验该 JSONL，`render-chat` 可生成本地 HTML 供回看；它们不从聊天应用直接导出数据，也不自动解析任意平台格式。聊天中的陈述仍需作为来源内容核查。

**Computer History（可选）**：如果你已在 Codex 中启用 Computer History，可以明确授权 Agent 查找与本次记录相关的电脑活动片段，用于找回曾浏览的网页、文档或消息线索。使用时先确认记录状态与时间范围，再回到原始材料核验；电脑操作或访问时间不等于校园事件发生时间，自动摘要也不作为独立来源。

Computer History 不由本仓库捆绑或自动启用，本项目没有活动流自动导入或后台监测接口。只整理本次任务必要且允许公开的线索，不把完整活动流、无关输入或私人内容直接提交公开仓库。
...
- **保留线索，也核对线索**：EXIF 中的拍摄时间等信息可以辅助核查，但可能缺失或被修改；不要仅凭文件时间、EXIF、截图或自动摘要确定事件的时间、地点与真实性。
- **原件与公开副本分开**：EXIF 或视频元数据可能带有精确位置等私人信息。保留原件供核查，公开前审查文字、画面与元数据；必要时仅发布处理后的副本或来源元数据。当前工具不会自动解析、核验或清除图片和视频中的元数据，扫描通过不代表原件可以直接公开。
...
**想补充资料或更正记录**：不必从写代码开始。你可以在 [GitHub Issues](https://github.com/Shuang-su/sztu-connect/issues) 提供可公开的事件线索、出处和必要说明，补充、反驳或更正既有论断；也可以按照 [贡献指南](CONTRIBUTING.md) 在自己的仓库中整理后提交修改。请先核对 [事实核查要求](docs/FACT_CHECKING.md) 与 [隐私边界](docs/PRIVACY.md)，不要把敏感原件或完整活动记录直接贴进公开 Issue。
