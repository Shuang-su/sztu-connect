...
附带工具任务的独立工作副本已有 `importers/registry.json` 草稿，但尚未进入实施分支。仅据其已观察到的 `schema_version: 1` 结构制作适配与合成测试，不复制另一任务的未完成文件，也不自行替换工具。最终平台、文件路径、固定版本与哈希仍须由该任务的正式交付核对。
...
一手文档足以确定共用指南、薄适配、宿主环境确认和本地初始化助手的设计；继续搜索无法替代实机安装。剩余缺口是正式工具归档交付、真实 Windows 运行、九客户端版本与模式逐项验收，以及指南进入默认分支后的公开 URL 检查。它们在 [验收矩阵](../../ONBOARDING_TEST_MATRIX.md) 中保持未完成状态，不由静态兼容性推断为已支持。
