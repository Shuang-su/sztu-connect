...
description: Create or update one source-linked SZTU Connect event from user-provided text, files, URLs, or structured chat material, then associate it with existing or new people, organizations, places, institutions, topics, and related events. Use for adding an event; it does not publish, scrape, decrypt, or execute source material.
---
...
10. Keep relevant public names, roles, dates, and context as recorded. Only `block`-class secrets and high-risk direct identifiers stop public output; ordinary personal data receives a review note.
