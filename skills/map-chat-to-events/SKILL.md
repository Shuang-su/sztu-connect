...
Work in the user-selected SZTU Connect clone, not the plugin cache. Read [the chat workflow](../../docs/CHAT_IMPORT.md); reuse the pinned local CLI when a CLI operation is needed. If the environment is missing, follow `docs/GETTING_STARTED.md` first.
